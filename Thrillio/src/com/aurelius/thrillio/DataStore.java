...
//				"", 2007, "Prentice Hall", new String[] { "Joshua Bloch" }, BookGenre.PHILOSOPHY, 4.9);
		...
		for (String row : data) {
			...
			bookmarks[2][colNumber++] = BookmarkManager.getInstance().createBook(Long.parseLong(values[0]), values[1],
...
//				MovieGenre.FOREIGN_MOVIES, 8.4);
		...
		for (String row : data) {
			...
			bookmarks[1][colNumber++] = BookmarkManager.getInstance().createMovie(Long.parseLong(values[0]), values[1],
	...
	private static void loadWebLinks() {
		...
		for (String row : data) {
			String[] values = row.split("\t");
			bookmarks[0][colNumber++] = BookmarkManager.getInstance().createWebLink(Long.parseLong(values[0]),
